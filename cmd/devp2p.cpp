...
#include "common/devp2p_options.hpp"
...
Settings devp2p_parse_cli_settings(int argc, char* argv[]) {
    CLI::App cli{"devp2p - devp2p utility"};
    ...
    add_devp2p_options(cli, settings);
...
void sentry_main(Settings settings) {
    ...
    log::Info() << "devp2p is now running [pid=" << pid << ", main thread=" << tid << "]";
    ...
    log::Info() << "devp2p exiting [pid=" << pid << ", main thread=" << tid << "]";
}

int main(int argc, char* argv[]) {
    try {
        sentry_main(devp2p_parse_cli_settings(argc, argv));
    } catch (const CLI::ParseError& pe) {
        return -1;
    } catch (const std::exception& e) {
        log::Critical() << "devp2p exiting due to exception: " << e.what();
        return -2;
    } catch (...) {
        log::Critical() << "devp2p exiting due to unexpected exception";
        return -3;
    }
}
